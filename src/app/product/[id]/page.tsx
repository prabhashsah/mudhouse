...
import { ShoppingBag, ChevronLeft, Star, Coffee, Info, Loader2 } from "lucide-react";
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      if (!id) return;
      try {
        const docSnap = await getDoc(doc(db, "items", id as string));
        if (docSnap.exists()) {
  ...
  if (loading) {
    return (
      <div className="pt-32 pb-24 bg-sand min-h-screen flex items-center justify-center">
